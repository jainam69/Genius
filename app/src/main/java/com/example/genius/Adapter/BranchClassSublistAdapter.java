...
public class BranchClassSublistAdapter extends RecyclerView.Adapter<BranchClassSublistAdapter.ViewHolder> {

    Context context;
    public List<BranchClassSingleModel.BranchClassData> CourceDataList;

    public BranchClassSublistAdapter(Context context, List<BranchClassSingleModel.BranchClassData> courceDataList) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.row_branch_class_sublist_line, parent, false));
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (CourceDataList.get(position).isClass) {
            holder.linear_class_sublist.setVisibility(View.VISIBLE);
            holder.txt_class_name.setText(CourceDataList.get(position).classModel.getClassName());
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView txt_class_name, txt_selected;
